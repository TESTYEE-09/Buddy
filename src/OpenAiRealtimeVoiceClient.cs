...
namespace LethalAICrewmate
{
    ...
    internal static class OpenAiRealtimeVoiceClient
    {
        ...
        private const string ToolDefinitionsJson =
            "{\"type\":\"function\",\"name\":\"move_buddy\",\"description\":\"Move Buddy ONLY when the speaker actually orders it right now: follow, stay, return to ship, fetch scrap, or scout ahead. A polite request (\\\"can you grab that?\\\", \\\"could you follow me?\\\") IS an order - call it. But a question about the job rather than for it is conversation - never call for \\\"ready to get the scrap?\\\", \\\"do you even fetch scrap?\\\" or similar. Also never call for plans, hypotheticals, complaints, negated requests, or reports of an action already taken. Going into the facility is never one of these moves: \\\"come inside with me\\\" or \\\"follow me in\\\" reads like follow, but Buddy refuses it in one line and no call is made. fetch_scrap picks the nearest worthwhile loose scrap on its own; pass item_name only when the speaker names a specific item.\",\"parameters\":{\"type\":\"object\",\"properties\":{\"action\":{\"type\":\"string\",\"enum\":[\"follow\",\"stay\",\"return_to_ship\",\"fetch_scrap\",\"scout_ahead\"]},\"distance_metres\":{\"type\":\"number\",\"description\":\"Scout distance, normally 4 to 18 metres.\"},\"bring_to_player\":{\"type\":\"boolean\",\"description\":\"For fetch_scrap only: deliver to the requesting player instead of the ship.\"},\"item_name\":{\"type\":\"string\",\"description\":\"For fetch_scrap only: the item the speaker named, e.g. 'bolt' or 'propane'. Omit to fetch the nearest worthwhile loose scrap.\"}},\"required\":[\"action\"]}}," +
